...
(function (window, document, $, undefined) {
	...
	$.extend(F, {
		...
		defaults: {
			...
			// HTML templates
			tpl: {
				wrap     : '<div class="fancybox-wrap" tabIndex="-1"><div class="fancybox-skin"><div class="fancybox-outer"><div class="fancybox-inner"></div><div class="fancybox-custom clearfix"><div class="fancybox-right-col"></div></div></div></div></div>',
		...
		_start: function (index) {
			...
			$.extend(coming, {
				...
				custom : $('.fancybox-custom', coming.wrap)
			});
	...
	F.helpers.title = {
		beforeShow: function (opts) {
			...
			index = $('<div class="fancybox-index fancybox-index-' + type + '-wrap">' + ( F.current.index + 1) + ' of ' + F.current.group.length + '</div>');
			caption = $('<div class="fancybox-caption">' + $(F.current.element).data('caption') + '</div>');
			image = $('<div class="fancybox-custom-image"><img src="' +$(F.current.element).attr('href')+ '"></div>');

			if (window.console) {console.log($(F.current.element).data('caption'));}
			...
			if (opts.position === 'top') {
				...
				target.find('.fancybox-right-col').append(index, title, caption);
				$('.fancybox-inner').remove();
			} else {
				title.appendTo(target);
				index.prependTo(target);
